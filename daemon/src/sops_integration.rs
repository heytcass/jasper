    ...
    fn decrypt_sops_file(path: &Path) -> Result<Self> {
            ...
            .output()
        {
            if output.status.success() {
                ...
                debug!(
                    "Nix develop sops command failed: {}",
                    String::from_utf8_lossy(&output.stderr)
                );
            }
        }
        ...
        // All methods failed
        Err(anyhow!(
            "Could not decrypt SOPS file using any available method: {}",
            path_str
        ))
    }
        ...
        for line in yaml_content.lines() {
            ...
            if let Some(colon_pos) = line_content.find(':') {
                ...
                if value.is_empty() {
                    ...
                    if !full_key.starts_with("sops")
                        && !full_key.contains("lastmodified")
                        && !full_key.contains("mac")
                    {
